...
| Orbit | 尚未上架；计划以个人账号发布 | 尚无 App Store 卖家主体。若个人加入，卖家名会是你的法定姓名；若组织加入，才显示组织法定名称。[^1] | 尚无 | 当前工程目标为 iOS 15+；GitHub Actions 已提供侧载与签名构建路径 | 默认 Orbit 账户 + 云端积分；BYOK 为高级可选项 |
...
| 手动新建、编辑、删除事件 | ✔ | ✔ | ✔ | ✔ |
| 文字自然语言转日程 | Siri 指令/建议，非聊天式批量解析 | ✔ | ✔ | ✔，默认云端积分；高级用户可自带 Key |
...
| 账号、后端额度、订阅权益 | Apple Account/系统服务 | ✔ | ✔ | **账户、积分和消耗型 IAP 已接入；正式部署与交易验收待完成** |
...
| P0 | 正式 Team、后端生产环境与 IAP 配置尚未完成 | 无法作为正式 App Store 身份、安全运营云端积分 | 开发者账号生效后配置 Team、证书、后端密钥与 StoreKit 产品；完成生产交易、退款和审计验收 |
